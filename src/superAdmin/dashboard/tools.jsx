...
export function Tools() {
	...
	const addtool = () => {
		...
		axios
			.post(`${api}work-tool/add`, addData, config)
			.then(res => {
				closeAddModal();
				gettool();
	...
	const edittool = () => {
		...
		axios
			.put(
				`${api}work-tool/edit?id=${ToolData ? ToolData.id : 0}`,
				editData,
				config
			)
			.then(res => {
	...
	const deletetool = () => {
		axios
			.delete(`${api}work-tool/delete?id=${ToolData ? ToolData.id : 0}`, config)
			.then(() => {
				closeDeleteModal();
				gettool();
