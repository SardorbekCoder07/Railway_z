...
export function AddLocation() {
	...
	const [railwayId, setRailwayId] = useState(null);
	...
	const getkm = id => {
		axios
			.get(`${api}railway/list?pdbId=${id}`, config)
			.then(res => {
				if (res.data.success && res.data.body.length !== 0)
					setkm(res.data.body);
				else setkm(null);
	...
	const handleDelete = async () => {
		try {
			await axios.delete(`${api}railway/${railwayId}`, config);
			toggleModal();
			getkm(selectPdbId);
			toast.success("Ish quroli muvoffaqqiyatli o'chilrildi!👌");
		} catch (error) {
			toast.error('Xatolik yuz berdi ❗️');
		}
	};
						...
						<div className='flex justify-center flex-col items-center gap-7'>
							<div className='w-full max-w-[24rem]'>
								<Input
									...
									onChange={editRegex}
									required
									defaultValue={kmData ? kmData.km : "Ma'lumot yo'q"}
									id='editkm'
									label='km nomi'
								/>
							</div>
