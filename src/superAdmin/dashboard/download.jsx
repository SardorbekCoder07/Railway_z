...
const Download = ({isModalDown, closeDown}) => {
    ...
    const downloadExcelFile = () => {
        ...
        if (addData.date) {
            axios.post(`${api}download?date=${addData.date}`, '', {...config, responseType: 'blob'})
                .then((res) => {
                    const blob = new Blob([res.data], {type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});
                    const url = window.URL.createObjectURL(blob);
                    const a = document.createElement('a');
                    a.href = url;
                    a.download = `Railway-${addData.date}.xlsx`;
        ...
        <Dialog open={isModalDown} onClose={closeDown}>
            ...
            <DialogBody>
                <div className="relative h-10 w-full min-w-[200px]">
                    ...
                    <p className={`text-red-600 font-semibold mb-5 ${error === '' ? 'hidden' : 'block'}`}>{error}</p>
                </div>
            </DialogBody>
            <DialogFooter>
                <Button onClick={() => {
