...
export function PdUsers() {
	...
	const getUser = () => {
		axios
			.get(`${api}user/leader/no/pd`, config)
			.then(res => {
	...
	const addPD = () => {
		...
		if (userID) {
			axios
				.post(`${api}pd/add`, addData, config)
				.then(res => {
					closeAddModal();
					getPD();
					getUser();
	...
	const editPd = () => {
		...
		axios
			.put(`${api}pd/update?id=${userData ? userData.id : 0}`, editData, config)
			.then(res => {
				closeEditModal();
				getPD();
	...
	const deletePD = () => {
		axios
			.delete(`${api}pd/delete?id=${userData ? userData.id : 0}`, config)
			.then(() => {
				closeDeleteModal();
				getPD();
	...
	const addRegex = () => {
		...
		const employeeCountValue = document
			.getElementById('addemployeeCount')
			.value.trim();
	...
	const editRegex = () => {
		...
		const employeeCountValue = document
			.getElementById('editemployeeCount')
			.value.trim();
						...
						<div className='flex justify-center flex-col items-center gap-7'>
							<div className='w-full max-w-[24rem]'>
								<Input
									onChange={editRegex}
									required
									defaultValue={userData ? userData.name : "Ma'lumot yo'q"}
									id='editPD'
									label="Bo'linma nomi"
								/>
							</div>
							<div className='w-full max-w-[24rem]'>
								<Input
									...
									required
									defaultValue={
										userData ? userData.employeeCount : "Ma'lumot yo'q"
									}
